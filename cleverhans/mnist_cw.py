...
def mnist_tutorial_cw(train_start=0, train_end=60000, test_start=0,
    ...
    for i in range(length // batch_size):    
        adv = cw.generate_np(X_test[i*batch_size:i*batch_size+batch_size],
                         **cw_params)
        if i==0:
            adv_test = adv[:]
        else:
            adv_test = np.concatenate((adv_test,adv),axis=0)
    ...
    for i in range(length // batch_size):    
        adv = cw.generate_np(X_train[i*batch_size:i*batch_size+batch_size],
                         **cw_params)
        if i==0:
            adv_train = adv[:]
        else:
            adv_train = np.concatenate((adv_train,adv),axis=0)
