...
from defensegan_datasets.utils import get_generators
from defensegan_models.base_model import AbstractModel
from defensegan_models.dataset_models import mnist_generator, celeba_discriminator, \
...
class DefenseGANBase(AbstractModel):
    ...
    def _loss(self):
        ...
        if self.mode == 'wgan':
            ...
            clip_ops = []
            for var in slim.get_variables('Discriminator'):
                clip_bounds = [-.01, .01]
                clip_ops.append(
                    tf.assign(
                        var,
...
GAN = MnistDefenseGAN(load_cfg('./DefenseGAN-cfg.yml'), test_mode = False)
    
    
def DefenseGANLayer(x):
    return gan.reconstruct(x, batch_size=128)


def DefenseGAN():
    model = Sequential()
    model.add(Lambda(DefenseGANLayer))
    return model